...
Inicialize o Git no seu diretório de projeto:
...
Adicione o repositório remoto (ex: GitHub, GitLab):
...
Renomeie a branch principal (opcional, mas recomendado):
...
git clone https://github.com/danielbellinijf/todolistJava.git
...
Edite o arquivo .env que você acabou de criar e preencha as variáveis de acordo com suas necessidades. Certifique-se de substituir os valores seu_usuario, sua_senha e SuaChaveSecretaUnicaAquiParaJWT por seus dados reais:
...
Registrar um Usuário: Envie uma requisição para o endpoint de registro.

Fazer Login: Após o registro, envie uma requisição para o endpoint de login com as credenciais do usuário. A resposta conterá um token JWT.

Usar o Token: Para acessar endpoints protegidos, inclua o token JWT no cabeçalho Authorization de suas requisições, no formato Bearer <seu_token_jwt>.
...
Lista todas as tarefas do usuário autenticado, com filtros opcionais por status, prioridade e data de expiração.
...
Atualiza o status de uma tarefa específica por ID.
...
Exclui uma tarefa por ID e todas as suas subtarefas associadas (cascata).
...
Cria uma nova subtarefa associada a uma tarefa principal (taskId).
...
Atualiza o status de uma subtarefa específica por ID.
...
Exclui uma subtarefa específica por ID.
...
testValidatePasswordCorrectly: Valida que a comparação de senhas retorna true quando a senha bruta corresponde à senha criptografada.

testValidatePasswordIncorrectly: Verifica que a comparação de senhas retorna false quando a senha bruta não corresponde à senha criptografada.
...
🤝 Contribuindo
Contribuições são bem-venidas! Se você quiser contribuir com este projeto:

Faça um fork do repositório.

Crie uma nova branch (git checkout -b feature/sua-feature).

Faça suas alterações e commite-as (git commit -m 'feat: adiciona nova feature').
